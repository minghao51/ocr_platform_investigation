...
def _count_populated_fields(data: dict | None) -> Dict[str, Any]:
    if not data:
...
def _assess_file(file_path: str, file_type: str) -> Dict[str, Any]:
    quality_score = None
    doc_type = None
...
async def run_ab_comparison(
    ...
    file_type = "document" if Path(file_path).suffix.lower() == ".pdf" else "image"
    file_assessment = _assess_file(file_path, file_type)
    ...
    for _ in range(iterations):
        b = await _run_single_extraction(
            file_path, schema_definition, baseline_prompt, baseline_system,
            provider_name, model, extraction_method,
        )
        baseline_runs.append(b)

        o = await _run_single_extraction(
            file_path, optimized_schema, optimized_prompt, optimized_system,
            provider_name, model, extraction_method,
        )
    ...
    def _aggregate(runs: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "data": merged,
            "field_stats": _count_populated_fields(merged),
        }
    ...
    prompt_sizes = {
        "baseline_user_tokens": _count_tokens_approx(baseline_prompt),
        "optimized_user_tokens": _count_tokens_approx(optimized_prompt),
        "optimized_system_tokens": _count_tokens_approx(optimized_system or ""),
    ...
    return {
        ...
        "comparison": {
            "completeness_delta": round(
                ...
                - baseline_agg["field_stats"]["completeness"], 3
            ),
...
def _print_report(report: Dict[str, Any]) -> None:
    ...
    print(f"\n{'='*70}")
    print(f"  PROMPT OPTIMIZER BENCHMARK REPORT")
    print(f"{'='*70}")
    ...
    print(f"{'─'*70}")
    print(f"  {'Success rate':<28} {b['success_rate']:>14.1%} {o['success_rate']:>14.1%}")
    print(f"  {'Avg time (s)':<28} {b['avg_time_seconds']:>14.3f} {o['avg_time_seconds']:>14.3f} {c['time_delta_seconds']:>+14.3f}")
    print(f"  {'Field completeness':<28} {b['field_stats']['completeness']:>14.1%} {o['field_stats']['completeness']:>14.1%} {c['completeness_delta']:>+14.1%}")
    print(f"  {'Populated fields':<28} {b['field_stats']['populated']:>14} {o['field_stats']['populated']:>14} {o['field_stats']['populated'] - b['field_stats']['populated']:>+14}")

    if acc:
        print(f"{'─'*70}")
        if "baseline" in acc:
            print(f"  {'Accuracy (overall)':<28} {acc['baseline']['overall_score']:>14.1%}", end="")
        if "optimized" in acc:
            delta = acc["optimized"]["overall_score"] - acc.get("baseline", {}).get("overall_score", 0)
            print(f" {acc['optimized']['overall_score']:>14.1%} {delta:>+14.1%}")
        else:
            print()
        if "baseline" in acc:
            print(f"  {'Fields matched (≥0.85)':<28} {acc['baseline']['matched_fields']:>14}/{acc['baseline']['total_fields']}", end="")
        if "optimized" in acc:
            print(f" {acc['optimized']['matched_fields']:>14}/{acc['optimized']['total_fields']}")
        else:
            print()

    sizes = report["prompt_sizes"]
    print(f"{'─'*70}")
    print(f"  {'Prompt tokens (user)':<28} {sizes['baseline_user_tokens']:>14} {sizes['optimized_user_tokens']:>14}")
    print(f"  {'System prompt tokens':<28} {'N/A':>14} {sizes['optimized_system_tokens']:>14}")
...
async def run_dataset_benchmark(
    ...
    samples: List[BenchmarkSample] = datasets_extended.load_dataset("cord", max_samples=max_samples)
...
async def main() -> None:
    ...
    parser.add_argument("--schema", default="Generic", help="Schema name (Invoice, Receipt, ID, Generic)")
    parser.add_argument("--expected", default=None, help="JSON file or inline JSON with expected results")
    ...
    parser.add_argument("--iterations", type=int, default=1, help="Number of iterations per variant")
    ...
    if args.dataset:
        results = await run_dataset_benchmark(
            args.provider, args.model, args.method, args.max_samples,
        )
        output_path = args.output or f"benchmark_prompt_optimizer_{args.dataset}_{int(time.time())}.json"
    ...
    output_path = args.output or f"benchmark_prompt_{args.schema.lower()}_{int(time.time())}.json"
    with open(output_path, "w") as f:
