...
def _build_ablation_prompt(
    ...
    """
    resolved_doc_type = doc_type or classify_doc_type_hint(
    ...
    if condition == "cot":
        use_cot = resolved_doc_type == "handwritten" or (
            quality_score is not None and quality_score < 50.0
        )
        user_parts = [base_prompt]
        if use_cot:
...
async def _run_extraction(
    ...
    return await run_single_extraction(
        file_path,
        schema_definition,
        prompt,
        system_prompt,
    ...
    for condition in ABLATION_CONDITIONS:
        if condition == "full":
            ...
            pr = _build_ablation_prompt(
                ...
                schema_definition,
                schema_name,
                file_assessment.get("doc_type"),
                file_assessment.get("quality_score"),
            )
            schema_to_use = (
                pr.enriched_schema if pr.enriched_schema else schema_definition
            )
        ...
        stats = count_populated_fields(ext.get("data"))
        accuracy = None
        ...
        condition_results[condition] = {
            ...
            "user_tokens": count_tokens_approx(pr.user_prompt),
            "system_tokens": count_tokens_approx(pr.system_prompt),
        }
    ...
    for i, report in enumerate(reports):
        ...
        for cond_name, cond in conditions.items():
            await crud.add_benchmark_result(
                ...
                expected_json=json.dumps(cond.get("data"))[:4000]
                if cond.get("data")
                else None,
                actual_json=None,
                field_scores=json.dumps(
                    {
                        ...
                        "accuracy": float(cond["accuracy"])
                        if cond.get("accuracy") is not None
                        else None,
                    },
                    default=str,
                ),
...
def _print_ablation_report(report: Dict[str, Any]) -> None:
    ...
    print(f"{'─' * 80}")
    print(
        f"  {'Condition':<18} {'Success':>8} {'Time':>8} {'Complete':>10} {'Accuracy':>10} {'Tokens':>8}"
    )
    print(f"{'─' * 80}")
...
def _print_summary_matrix(
    all_reports: List[Dict[str, Any]], providers: List[str]
) -> None:
    print(f"\n{'=' * 90}")
    print("  ABLATION SUMMARY — ALL SAMPLES")
    print(f"{'=' * 90}")

    for provider in providers:
        provider_reports = [
            r for r in all_reports if r["config"]["provider"] == provider
        ]
        ...
        print(f"\n  Provider: {provider}")
        print(
            f"  {'Condition':<18} {'Avg Acc':>10} {'Avg Time':>10} {'Avg Compl':>10} {'Success%':>10} {'Samples':>8}"
        )
        print(f"  {'─' * 76}")

        for cond in ABLATION_CONDITIONS:
            accs = [
                r["conditions"][cond].get("accuracy")
                for r in provider_reports
                if r["conditions"][cond].get("accuracy") is not None
            ]
            times = [r["conditions"][cond]["elapsed_seconds"] for r in provider_reports]
            compls = [
                r["conditions"][cond]["field_stats"]["completeness"]
                for r in provider_reports
            ]
            ...
            print(
                f"  {cond:<18} {avg_acc:>10} {avg_time:>10} {avg_compl:>10} {succ_pct:>10} {len(provider_reports):>8}"
            )
...
async def main() -> None:
    ...
    parser.add_argument(
        "--expected", default=None, help="Ground truth JSON or @file.json"
    )
    parser.add_argument(
        "--providers", nargs="+", default=["gemini"], help="Providers to test"
    )
    parser.add_argument(
        "--model",
        default=None,
        help="Model name (auto-detected per provider if omitted)",
    )
    ...
    if not args.no_db:
        for provider in args.providers:
            ...
            if provider_reports:
                ...
                print(
                    f"Persisted {len(provider_reports)} results to benchmark_runs.id={run_id}"
                )

    output_path = (
        args.output or f"ablation_{args.dataset or 'single'}_{int(time.time())}.json"
    )
