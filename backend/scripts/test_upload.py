...
def test_upload(file_path: str, api_url: str = "http://localhost:8000"):
    ...
    if file_size > max_size:
        print(
            f"❌ Error: File too large ({file_size / 1024 / 1024:.2f}MB). Max size: 10MB"
        )
        sys.exit(1)
    ...
    if file_ext not in valid_extensions:
        print(
            f"❌ Error: Invalid file type '{file_ext}'. Valid types: {', '.join(valid_extensions)}"
        )
        sys.exit(1)
    ...
    try:
        ...
        with open(file_path, "rb") as f:
            files = {"file": (file_path_obj.name, f, mime_types[file_ext])}
...
Examples:
  ...
  python test_upload.py document.png --verbose
    ...
    parser.add_argument("file", help="Path to the file to upload (JPG, PNG, PDF)")
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Display full API response"
    )
